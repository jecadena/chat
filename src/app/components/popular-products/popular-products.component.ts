...
export class AppPopularProductsComponent implements OnInit {
  ...
  fetchPedidos() {
    ...
    this.http.get<PedidosResponse>('http://192.168.1.105:3000/api/pedidos/user', { headers })
      .subscribe(
        (data) => {
  ...
  onGeneratePedido(form: any) {
    ...
    this.http.post('http://192.168.1.105:3000/api/pedidos', { det_pedido: form.value.detPedido }, { headers })
      .subscribe(
        (response: any) => {
  ...
  onDeletePedido(id: string) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.http.delete(`http://192.168.1.105:3000/api/pedidos/${id}`, { headers })
          .subscribe(
            () => {
              Swal.fire({
                icon: 'success',
