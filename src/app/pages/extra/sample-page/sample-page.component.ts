...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  loadMessages(): void {
    ...
    this.http
      .get<{ messages: { message: string; userId: string; nombres: string; apellidos: string; timestamp: string }[] }>(
        `http://192.168.1.105:3000/api/messages/${this.roomId}`,
        { headers }
      )
      .subscribe(
        (response) => {
  ...
  sendMessage(): void {
    if (this.message.trim()) {
      ...
      this.http
        .post(
          'http://192.168.1.105:3000/api/sendMessage',
          {
            roomId: this.roomId,
      ...
      this.chatService.sendMessageToSocket(this.roomId, this.message, this.userId);
    }
  }
